...
# ── LATE-ENTRY STRATEGY (separate experiment, modeled on the validated paper
# trader: enter in the final ~15s with momentum + reversal filters and a near-
# zero move threshold). These re-enable filters that normal maker mode bypasses.
# All default OFF so this file behaves exactly like the standard maker bot
# unless LATE_ENTRY=true is set. Honest caveat: paper trading assumed fills and
# settled on its own feed; real money fills only if a seller shows and settles
# on Chainlink (~3s off Binance). Expect the smallest-move windows to be where
# real losses first appear.
LATE_ENTRY               = os.environ.get("LATE_ENTRY", "false").lower() == "true"
# When LATE_ENTRY is on, require the momentum check (last N ticks all moving the
# bet's way) before resting a bid. This is the paper trader's real quality gate.
LATE_REQUIRE_MOMENTUM    = os.environ.get("LATE_REQUIRE_MOMENTUM", "true").lower() == "true"
LATE_MOMENTUM_TICKS      = int(os.environ.get("LATE_MOMENTUM_TICKS", "3"))
# When LATE_ENTRY is on, still apply the reversal/choppy guards in maker mode
# (normal maker mode skips them; the paper trader kept them).
LATE_APPLY_REVERSALS     = os.environ.get("LATE_APPLY_REVERSALS", "true").lower() == "true"
...
def process_tick():
    for tf in TIMEFRAMES:
        ...
        for asset in ASSET_LIST:
            ...
            if ((not MAKER_MODE) or (LATE_ENTRY and LATE_APPLY_REVERSALS)) and revs > max_revs:
                w["skipped"] = True
                tg(
                    f"⏭ <b>SKIPPED · {emoji} {asset} · {tf}m · {dirn}</b> {arrow}\n\n"
                    f"📈 Move: {pct:+.3f}%\n"
            ...
            if ((not MAKER_MODE) or (LATE_ENTRY and LATE_APPLY_REVERSALS)) and recent_revs >= choppy_limit:
                w["skipped"] = True
                tg(
                    f"⏭ <b>SKIPPED · {emoji} {asset} · {tf}m · {dirn}</b> {arrow}\n\n"
                    f"📈 Move: {pct:+.3f}%\n"
            ...
            # LATE-ENTRY momentum gate: only rest a bid if the last N ticks are
            # all moving our way. This is the paper trader's real quality filter
            # (it replaces the move-size threshold, which paper proved redundant).
            if LATE_ENTRY and LATE_REQUIRE_MOMENTUM:
                if not check_momentum(hist, dirn, LATE_MOMENTUM_TICKS):
                    w["skipped"] = True
                    log.info(f"[LATE] skip {asset} {tf}m {dirn} - momentum not confirmed (last {LATE_MOMENTUM_TICKS} ticks)")
                    continue
...
def main():
    ...
    if LATE_ENTRY:
        mode_str += (f"\n🕯 LATE-ENTRY: last {CONFIG['entry_window_seconds']}s · "
                     f"momentum {'ON' if LATE_REQUIRE_MOMENTUM else 'off'}({LATE_MOMENTUM_TICKS}) · "
                     f"reversals {'ON' if LATE_APPLY_REVERSALS else 'off'}")
